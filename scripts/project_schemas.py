#!/usr/bin/env python
"""Hand-controlled Pydantic -> JSON Schema projector (design D13).

This is the single source of the emitted wire form: models are the authoritative authoring
surface, and this projector (not raw emitter defaults piped to disk) writes the canonical
Draft 2020-12 schemas under ``schemas/contracts/v1/`` with a version-pinned ``$id``. Run with
``--check`` in CI to fail on drift between the models and the committed schemas.
...
def _project(name: str, model: type[BaseModel]) -> dict[str, Any]:
    ...
    projected: dict[str, Any] = {
        ...
        "$comment": f"generated by nel-contracts projector {version}",
    }
